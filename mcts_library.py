...
class MCTS(object):
    ...
    def __init__(self, computation_budget, belief, initial_pose, rollout_length, path_generator, aquisition_function, f_rew, T, scheduler=None, grid=None, aq_param = None, use_cost = False, tree_type = None, num_samples=20):
        '''
        ...
        #POMPCPOW initialization
        self.k_action = 5
        self.alpha_action = 0.5
    ...
    def choose_trajectory(self, t, ratio_U = None):
        ...
        time_end = time.time()
        print ("Rollouts completed in", str(time_end - time_start) +  "s", best_sequence, best_val, all_vals = self.get_best_child())
    ...
    def initialize_tree(self):
        ...
        if self.scheduler is not None:
            self.rl = self.scheduler.get_H(self.GP, self.grid)
            
        else:
            self.rl = self.base_rl
        tree = {}
    ...
    #def tree_policy(self):
        ...
        N = self.tree[node]["visits"]

        # Progressive widening condition
        max_children = int(self.k_action * (N ** self.alpha_action))

        if len(self.tree[node]["children"]) < max_children:

            # expand new action
            a = random.choice(list(actions.keys()))

            child_name = node + " child " + str(a)
            ...
            self.tree[node]["children"].append(child_name)
            ...
            for child in self.tree[node]["children"]:
                ...
                Q = self.tree[child]["reward"]
                n = self.tree[child]["visits"]
                N = self.tree[node]["visits"]

                ucb = Q + self.c * np.sqrt(np.log(N+1)/(n+1))
