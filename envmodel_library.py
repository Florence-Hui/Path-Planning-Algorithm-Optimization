    ...
    def __init__(self, ranges, NUM_PTS, variance, lengthscale, noise = 0.0001, 
	    ...
	    # Generate a set of discrete grid points, uniformly spread across the environment
            ...
            for T in range(self.time_duration):
                ...
                new_gp = GPModel(
                        ranges=self.GP.ranges,
                        lengthscale=self.GP.lengthscale,
                        variance=self.GP.variance,
                        noise=self.GP.noise
                        )

                new_gp.xvals = self.GP.xvals.copy()
                new_gp.zvals = self.GP.zvals.copy()

                new_gp.model = GPy.models.GPRegression(np.array(new_gp.xvals),np.array(new_gp.zvals),
                                new_gp.kern,noise_var=new_gp.noise)

                self.models[T] = new_gp
