...
class Evaluation:
    ...
    def plot_metrics(self):
        ...
        time = np.array(list(self.metrics['MSE'].keys()), dtype=float)
        ...
        simple_regret = np.array(list(self.metrics['simple_regret'].values()), dtype=float)
        ...
        for key, value in self.metrics.items():
            ...
            arr = arr.reshape(-1)
            numeric_metrics.append(arr)

        if len(numeric_metrics) > 0:
            metrics_array = np.vstack(numeric_metrics).T
            np.savetxt('./figures/' + self.reward_function + '/metrics.csv',
               metrics_array,
               delimiter=",")
        else:
            print("No numeric metrics to save.")
       ...
       #    f.close()
        ...
        time = range(len(self.horizon_history))
        plt.figure()
        plt.plot(time, self.horizon_history)
        plt.xlabel("Time step")
        plt.ylabel("Planning Horizon H(t)")
        plt.title("Adaptive Planning Horizon")
        plt.savefig("figures/horizon_vs_time.png")
        # fig, ax = plt.subplots(figsize=(8, 6))
